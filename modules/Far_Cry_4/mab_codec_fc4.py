"""Far Cry 4 MAB rotation codec - FC4-owned copy of the Disrupt 'aNi' codec.

Far Cry 4 and Watch Dogs 1 share Ubisoft's Disrupt-engine 'aNi' animation
codec.  These functions were previously imported from the watchdogs folder;
they are duplicated here so the Far Cry 4 import path is fully self-contained
...
def _bulk_key_pose(action, pb, frames, quats, locs=None):
    ...
    from .procedural_fc4 import fcurve_container
    n = len(frames)
    if n == 0:
        return
...
def apply_wd1_mab(context, mab, arm_obj, model_bones,
    ...
    from .quat_resample_fc4 import resample_rotation
    mult = 1
    ...
    if emulate_helpers:
        try:
            from .procedural_fc4 import emulate_procedural_helpers
