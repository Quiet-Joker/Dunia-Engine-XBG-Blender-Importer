...
from .chunks_fc2 import find_chunk
...
class XBGLodDistItemFC2(bpy.types.PropertyGroup):
...
class XBG_OT_ReadLODDistancesFC2(bpy.types.Operator):
    """Read the per-LOD switch distances from an XBG's SDOL chunk."""
    bl_idname = "xbg.read_lod_distances_fc2"
    ...
    def execute(self, ctx):
        ...
        if not dists:
            ...
            return {'CANCELLED'}
        sc.xbg_lod_dists_fc2.clear()
        for li, off, val in dists:
            it = sc.xbg_lod_dists_fc2.add()
            it.lod_index = li; it.offset = off; it.distance = val
        sc.xbg_lod_dist_path_fc2 = self.filepath
        sc.xbg_lod_dist_endian_fc2 = 'BE' if endian == BE else 'LE'
        self.report({'INFO'}, f"Loaded {len(dists)} LOD distances from "
                              f"{os.path.basename(self.filepath)}")
        return {'FINISHED'}


class XBG_OT_WriteLODDistancesFC2(bpy.types.Operator):
    """Write the edited LOD distances back into an XBG (in-place float patch)."""
    bl_idname = "xbg.write_lod_distances_fc2"
    ...
    def invoke(self, ctx, ev):
        src = ctx.scene.xbg_lod_dist_path_fc2
        if not src:
...
class XBG_PT_LODDistancePanelFC2(bpy.types.Panel):
    """View / edit the per-LOD switch distances of an XBG file."""
    bl_label = "LOD Distances"
    bl_idname = "OBJECT_PT_xbg_lod_distances_fc2"
    ...
    def draw(self, ctx):
        l = self.layout
        sc = ctx.scene
        l.operator("xbg.read_lod_distances_fc2", icon='IMPORT')
        if sc.xbg_lod_dist_path_fc2:
            l.label(text=os.path.basename(sc.xbg_lod_dist_path_fc2), icon='FILE')
            box = l.box()
            if not len(sc.xbg_lod_dists_fc2):
                box.label(text="No LODs", icon='INFO')
            for it in sc.xbg_lod_dists_fc2:
                box.prop(it, "distance", text=f"LOD {it.lod_index}")
            l.operator("xbg.write_lod_distances_fc2", icon='EXPORT')
