...
def _propagate(self, context):
    ...
    if apb is None or apb.xbg_jiggle_fc2 != self:
        return
...
class XBGJiggleBoneSettingsFC2(bpy.types.PropertyGroup):
...
def _bone_defs(pbones):
    """Build the bake `defs` dict from each bone's xbg_jiggle_fc2 settings."""
    defs = {}
    for pb in pbones:
        j = pb.xbg_jiggle_fc2
        lim = j.swing_limit
...
def _jiggle_targets(context):
    ...
    sel = context.selected_pose_bones or []
    marked = [pb for pb in sel if pb.xbg_jiggle_fc2.is_jiggle]
    return marked or sel


class XBG_OT_BakeJiggleFC2(bpy.types.Operator):
    """Bake the spring-bone jiggle preview onto the selected (jiggle) bones
    using their per-bone slider settings."""
    bl_idname = "xbg.bake_jiggle_fc2"
    ...
    def execute(self, ctx):
        ...
        for pb in targets:
            groups[round(pb.xbg_jiggle_fc2.strength, 4)].append(pb)
        total = 0
...
class XBG_OT_ClearJiggleFC2(bpy.types.Operator):
    """Remove baked jiggle keyframes from the selected bones and reset them."""
    bl_idname = "xbg.clear_jiggle_fc2"
...
def _patch_proceduralbones(src_path, dst_path, pawn_type, bone_settings, log=None):
    ...
    binary to dst.  `bone_settings` = {bone_name: PoseBone.xbg_jiggle_fc2}."""
    import xml.etree.ElementTree as ET
    from . import proceduralbones as pbmod
...
class XBG_OT_InjectJigglePatchFC2(bpy.types.Operator):
    ...
    bl_idname = "xbg.inject_jiggle_patch_fc2"
    ...
    def execute(self, ctx):
        from ..Core.prefs import get_prefs
        from .import_lks_fc2 import parse_lks_file
        ...
        if arm is None or arm.type != 'ARMATURE':
            self.report({'ERROR'}, "Select the armature first")
            return {'CANCELLED'}
        jbones = [pb for pb in arm.pose.bones if pb.xbg_jiggle_fc2.is_jiggle]
        if not jbones:
        ...
        skipped = []
        for pb in jbones:
            bone_settings[pb.name] = pb.xbg_jiggle_fc2
...
class XBG_PT_JigglePanelFC2(bpy.types.Panel):
    """Author + preview procedural jiggle bones."""
    bl_label = "Jiggle Bones"
    bl_idname = "OBJECT_PT_xbg_jiggle_fc2"
    ...
    def draw(self, ctx):
        ...
        if ctx.mode != 'POSE':
            ...
            l.operator("xbg.preview_jiggle_fc2", icon='PHYSICS',
        ...
        row.operator("xbg.bake_jiggle_fc2", icon='PLAY', text="Bake Preview")
        row.operator("xbg.clear_jiggle_fc2", icon='TRASH', text="Clear")
        l.label(text="Play the timeline to see the jiggle", icon='INFO')

        l.separator()
        box = l.box()
        n_marked = sum(1 for pb in arm.pose.bones if pb.xbg_jiggle_fc2.is_jiggle)
        box.label(text="Inject to game (%d jiggle bone(s)):" % n_marked,
                  icon='EXPORT')
        col = box.column()
        col.enabled = n_marked > 0
        col.operator("xbg.inject_jiggle_patch_fc2", icon='MODIFIER',
