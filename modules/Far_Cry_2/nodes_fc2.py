...
from .import_materials_fc2 import XBMMaterialData
...
def _image_is_grayscale(img, max_samples=4096, tol=0.04):
    """True when an image's RGB channels are (near-)equal everywhere.

    Used to decide where a specular map belongs on the Principled BSDF:
    a black-and-white map is an INTENSITY map (-> 'Specular IOR Level');
    only genuinely coloured maps carry a highlight tint (-> 'Specular
    Tint').  Errs toward grayscale when pixels can't be read."""
    try:
        ...
        px = px.reshape(-1, 4)
        idx = np.linspace(0, n - 1, min(max_samples, n)).astype(np.int64)
        s = px[idx]
        return bool(np.abs(s[:, 0] - s[:, 1]).max() < tol
                    and np.abs(s[:, 1] - s[:, 2]).max() < tol)
    except Exception:
        return True
...
class BlenderMaterialSetup:
    ...
    def _build_glass(ctx):
        ...
        *glowed* orange in every lighting condition (the "weird patchwork").
        ...
        # Orange specular highlight + environment-reflection tint.
