...
def _walk_lod(data, sdol_off, lod_idx):
    ...
    for li in range(n_lods):
        ...
        ne = s.u32()
        s.p += ne * 24                       # entries (Primal: 6×u32, not FC4's 7)
        s.u32()                              # vb_size
        s.align(16)
        vb_base = []
        vb_voff = []
...
def _extract_components(obj):
    ...
    # by ±1 (broke byte-exact zero-edit round-trips).  Verts whose attribute
    # entry is zero (e.g. added by an edit) fall back to corner normals.
    na = me.attributes.get('xbg_normal')
    have_attr = bool(na and na.domain == 'POINT' and len(na.data) == n)
    if have_attr:
        for vi in range(n):
            v = na.data[vi].vector
...
def _build_lod_payload(data, walk, lod, lods, lod_idx, overrides, palettes,
    ...
    for e in new_entries:
        # Primal entry = 6×u32 [vb, unk0, material, idx_start, last_vert,
        # byte_off] (internal list `e` keeps FC4's 7-field order — remap).
        vb_i, mat_i, _sub, idx0, last_v, byte_off, _res = e
        pay += struct.pack('<6I', vb_i, 0, mat_i, idx0, last_v, byte_off)
...
def inject_primal(context, objects, output_filepath, lod_idx=0):
