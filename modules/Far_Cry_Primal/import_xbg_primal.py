"""Far Cry Primal XBG mesh importer (Primal fork of the FC4 importer).

Primal (version 0x0006003A) is the SAME GEOM family as FC4 (0x00060037):
identical chunk set (LTMR/LEKS/EDON/MB2O/DIKS/DNKS+SULC/ITOM/SDOL/...),
identical flag-driven vertex layout (takkar_body uses the classic FC4
character flag 0x0BDA, stride 40).  ONE format difference found
(takkar_body_euro_m.xbg ground truth, byte-math verified): the SDOL
per-LOD section-entry records are **6 × u32 (24 B), not FC4's 7 × u32
(28 B)** — Primal dropped the trailing reserved 0, and the material index
moved to field 2:
    Primal entry: [vb_index, unk(0), material_index, idx_start,
                   last_vert, vert_byte_off]
    (verified: vert_byte_off/stride == previous entry's last_vert+1;
     material_index values 0..2 match the LTMR's exactly 3 materials;
     u32 right after n_entries*24 bytes == vb_size to the byte)
Entries are re-shaped into FC4's 7-field order after reading so all the
downstream section/material logic is unchanged.

Format support:
    * Far Cry Primal (version 0x0006003A) — takkar_body_euro_m.xbg + gear
...
VERSION_PRIMAL = 0x0006003A
VERSION_FC5 = 0x000D0047

SUPPORTED_VERSIONS = (VERSION_PRIMAL,)  # Primal-only; FC4 has its own folder
...
def _read_lod(s):
    ...
    for _ in range(n_entries):
        # Primal: 6 × u32 (24 B) — [vb, unk0, material, idx_start,
        # last_vert, vert_byte_off].  Re-shape to FC4's 7-field order
        # [vb, material, sub, idx_start, last_vert, byte_off, 0] so the
        # downstream section/material logic stays identical to FC4's.
        f0, f1, f2, f3, f4, f5 = struct.unpack_from('<6I', s.d, s.p)
        s.p += 24
        entries.append((f0, f2, f1, f3, f4, f5, 0))
...
def parse_xbg(path_or_bytes):
    ...
    if version not in SUPPORTED_VERSIONS:
        raise ValueError(
            ...
            f"(supported: Primal=0x{VERSION_PRIMAL:08x})"
        )
