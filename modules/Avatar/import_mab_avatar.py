...
def build_single_bone_timeline(d, sections):
    ...
    for g in groups:
        ...
        for ex_q, sub in zip(tp['extra'], bits):
            if ex_q is not None:
                timeline.append((base_frame + sub, ex_q))
...
def apply_single_bone(context, d, sections, arm_obj, bone_name,
    ...
    # playback of sparse / low-fps streams (see quat_resample_avatar).
    raw = []
    ...
    if mult > 1 and len(raw) >= 2:
        from .quat_resample_avatar import resample_rotation
...
def _fcurve_container(obj, action):
    ...
    from .procedural_avatar import fcurve_container
...
def apply_multi_bone(context, d, sections, arm_obj, xbg_path,
    ...
    n_helpers = 0
    if emulate_helpers:
        from .procedural_avatar import emulate_procedural_helpers
