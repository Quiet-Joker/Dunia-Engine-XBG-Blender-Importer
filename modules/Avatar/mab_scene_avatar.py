...
from .import_mab_avatar import parse_sections
...
def decode_and_tracks(d, sections):
    ...
    """
    from .import_mab_avatar import read_quat
    out = []
...
def find_scene_anchor_transform(path, anchor_name=None, root_rot=None):
    """World (location, rotation_quaternion or None) of an anchor (kind==7)
    in this scripted scene, or None if there's no usable anchor.

    `root_rot`: the scene-root world rotation to compose the anchor's
    parent chain under. Defaults to the character-placement frame
    (Rz(180°)) — the frame XBG_OT_ImportMAB snaps the character into.
    build_scene_objects passes the SCENE frame (Rz(90°)) as well so it can
    measure the offset between the two and shift the camera rig to sit on
    the character's anchor (they differ because the camera-track and
    bone-animation data are authored 90° apart about Z; see
    build_scene_objects).

    EXHAUSTIVELY CONFIRMED (2026-06-30) there is no character world-position
    data ANYWHERE in scripted-scene MABs — not in the Events scene-element
    table (only anchors/cameras/prop-attachments), not in UnkSec3 (already
    known to be face-bone + camera fine translation, not character
    placement), not in any of the 6 distinct timed-event types across the
    full 204-file scripted_event corpus (every AttachEvent attaches a PROP
    to a hand bone, never a character to a position). The level/trigger
    script that drives the actual cutscene positions characters BEFORE
    playing the clip - that's a different file format entirely, outside
    .mab. The closest approximation available from the file alone is the
    scene's own anchor system: 'centerscene' is the de-facto stand-mark in
    every file that has one. This is not a fallback compromise — it is the
    most complete answer obtainable from this file format.

    `anchor_name`: pick a specific anchor by name. None -> prefer
    'centerscene' if present, else the first kind==7 anchor. Walks the
    anchor's own parent chain (mirrors build_scene_objects' parenting) and
    applies the scene root's fixed 180-deg Z rotation, so the result is
    directly usable as a Blender object's world location/rotation.
    """
    if mathutils is None:
        raise RuntimeError("mathutils unavailable")
    import math
    d = open(path, 'rb').read()
    sec = parse_sections(d)
    elements = parse_scene_elements(d, sec)
    anchors = [el for el in elements if el['kind'] == 7]
    if not anchors:
        return None
    target = None
    if anchor_name:
        target = next((el for el in anchors if el['name'] == anchor_name), None)
    if target is None:
        target = next((el for el in anchors if el['name'] == 'centerscene'),
                      anchors[0])

    tracks = decode_and_tracks(d, sec)
    by_name = {el['name']: (el, tracks[i] if i < len(tracks) else None)
               for i, el in enumerate(elements)}

    if root_rot is None:
        root_rot = mathutils.Quaternion((0.0, 0.0, 1.0), math.pi)

    def local_transform(el, tr):
        loc = mathutils.Vector(tr['pos_static']) if tr and tr['pos_static'] else mathutils.Vector((0, 0, 0))
        rq = tr['rot_static'] if tr and tr['rot_static'] else None
        if rq:
            x, y, z, w = rq
    ...
    # Walk the parent chain up to the scene root, composing transforms —
    # same convention build_scene_objects uses for parenting elements.
    chain = []
    seen = set()
    cur_name = target['name']
    while cur_name and cur_name in by_name and cur_name not in seen:
        seen.add(cur_name)
        el, tr = by_name[cur_name]
        chain.append((el, tr))
        cur_name = el['parent']
    chain.reverse()   # root-most element first

    world_loc = mathutils.Vector((0, 0, 0))
    world_rot = root_rot
    for el, tr in chain:
        loc, rot = local_transform(el, tr)
        world_loc = world_loc + (world_rot @ loc)
...
def build_scene_objects(context, path, fps=30):
    ...
    `fps` is the PLAYBACK rate camera key times get converted to frame
    numbers against — it MUST match whatever `scene.render.fps` ends up
    being once a character is also driven by the same .mab via
    `apply_multi_bone` (modules/Avatar/import_mab_avatar.py), or the two
    animations desync (2026-06-30: found via a real "waking up" scripted
    scene — the character resamples to `ds.mab_resample_fps`, default 60,
    while this function silently assumed a hardcoded 30; once Blender's
    actual scene.render.fps got set to 60 by the character side, the
    camera's frame numbers - placed assuming 30 - played out at HALF
    their intended duration: camera finishes the shot while the body is
    still mid-animation). Callers MUST pass the same fps the character
    import will target (XBG_OT_ImportSceneMAB passes
    `ds.mab_resample_fps`); this function also sets `scene.render.fps`
    itself so whichever import runs first or last, they agree.
    """
    if bpy is None:
    ...
    d = open(path, 'rb').read()
    from .import_mab_avatar import parse_sections as _ps
    tracks = decode_and_tracks(d, _ps(d))

    # Scene-root world Z rotation. The whole camera/anchor rig hangs off
    # this empty at the world origin, so this single value rotates the
    # entire scene about the origin.
    #
    # It's the model importer's base 180° PLUS a -90° (clockwise, viewed
    # from top) correction (2026-06-30, user-diagnosed + geometrically
    # confirmed): the camera-TRACK data and the character's bone-ANIMATION
    # data are authored in coordinate frames that differ by 90° about Z
    # (Dunia camera convention vs. the skeleton convention). The base 180°
    # root is applied equally to both pipelines so it never accounts for
    # that 90° data-frame gap - the camera rig ends up 90° off from the
    # character even though the character sits right at the pivot. Verified
    # on the se_hellsgate_04_part2_avatar wake-up scene: character lies
    # along X (head at -X), camera sat at -Y (~-75° about Z, filming the
    # side); the -90° swings it to ~-165° (the -X head end) where a wake-up
    # shot belongs. The character is NOT touched (it's placed independently
    # via find_scene_anchor_transform / the anchor snap in
    # XBG_OT_ImportMAB) - only the camera/anchor rig rotates to meet it.
    #
    # If the camera ends up on the WRONG side (rotated the opposite way
    # from the character), flip the sign: `math.pi - math.pi / 2` (= +90°)
    # -> `math.pi + math.pi / 2` (= +270°). It's a pure Z-axis choice, one
    # of those two is correct.
    import math
    _CHAR_ROOT_Z = math.pi                       # 180° — the character-placement frame
    _SCENE_ROOT_Z = math.pi - math.pi / 2.0      # 180° base - 90° correction = 90°
    root = bpy.data.objects.new(base + ".scene", None)
    root.empty_display_type = 'ARROWS'
    root.rotation_euler = (0, 0, _SCENE_ROOT_Z)

    # Coincide the scene's anchor with the character's anchor (2026-06-30).
    # The character is snapped (XBG_OT_ImportMAB) to
    # find_scene_anchor_transform's result, computed in the 180° char frame;
    # the scene rig is rotated to 90° for camera alignment. Applying two
    # different rotations to the SAME anchor-local offset lands them ~0.15m
    # apart, so the camera orbits a point slightly off from where the
    # character actually sits. Fix: give the scene root a small LOCATION so
    # its anchor lands exactly on the character's anchor — keeps the 90°
    # rotation (needed for alignment), removes the positional slip.
    try:
        _char_a = find_scene_anchor_transform(
            path, root_rot=mathutils.Quaternion((0, 0, 1), _CHAR_ROOT_Z))
        _scene_a = find_scene_anchor_transform(
            path, root_rot=mathutils.Quaternion((0, 0, 1), _SCENE_ROOT_Z))
        if _char_a is not None and _scene_a is not None:
            root.location = _char_a[0] - _scene_a[0]
    except Exception:
        pass                                     # non-fatal: fall back to origin
