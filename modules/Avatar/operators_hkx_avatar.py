...
class XBG_OT_ImportHKXNative(bpy.types.Operator):
    """Import an Avatar .hkx collision file directly (binary, no XML)."""
    bl_idname  = "xbg.import_hkx_native"
    ...
    def execute(self, ctx):
        from .hkx_native_avatar import import_hkx_native
...
class XBG_OT_ExportHKXNative(bpy.types.Operator):
    """Write the edited collision back into a copy of the original .hkx."""
    bl_idname  = "xbg.export_hkx_native"
    ...
    def execute(self, ctx):
        from .hkx_native_avatar import export_hkx_native
        cs = ctx.scene.xbg_collision_settings
