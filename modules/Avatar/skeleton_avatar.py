...
class Skeleton:
    ...
    def _mb2o_root_frame(self):
        ...
        frame, not world space. (2026-06-30: previously only the
        translation was applied here, which was itself a side effect of the
        index-mapping bug below — once bones are matched to the right
        matrix, the rotation component is required too, see agents.md.)
        """
    ...
    def apply_bind_matrices(self, bind_matrices, sub_mesh_list=None):
        ...
        while walking DNKS submesh palettes" — this was verified WRONG by
        cross-referencing against EDON's already-correct world transforms
        (see agents.md "MB2O import-side verification", 2026-06-30): wrong
        assignments displaced bones by 1.3-2.3 *meters*, and in one case an
        entire bone's matrix landed on a completely different bone (Kendra's
        "Head Root" got "R Toe"'s data).

        EDON's hierarchy walk (compute_bone_transforms) already reconstructs
        every bone's correct bind-pose position independent of MB2O, so we
        use it as ground truth: each on-disk MB2O matrix is assigned to
        whichever EDON bone's own (already-correct) world position it lands
        closest to, once un-inverted into the root bone's frame. A correct
        assignment lands within ~0.3mm; a wrong one is off by 10cm-2m+, so a
        generous distance threshold cleanly separates real matches from
        noise. Matching is done as a single global greedy pass over ALL
        (mb2o_index, bone) pairs sorted by ascending distance, which avoids
        the "first bone claims a not-quite-best match, starving the real
        owner" failure mode of a naive per-index nearest-neighbor loop.

        Bones that don't get a confident MB2O match keep their EDON world
        transform (this is the existing/already-correct fallback - it's not
        a degraded case, EDON alone has been independently verified
        correct).
        """
...
def parse_skeleton_chunk(g, skeleton):
    ...
    for m in range(bone_count):
        ...
        name_len = g.i(1)[0]
        # Read the FULL bone name. The old `[-25:]` capped names at 25 chars,
        # which mangled longer ones (e.g. weapon parts:
        # 'DUAL_WASP_FRONT_ACCESSORIES' -> 'AL_WASP_FRONT_ACCESSORIES',
        # 'DUAL_WASP_LOCK_ACCESSORIES' -> 'UAL_WASP_LOCK_ACCESSORIES') and
        # broke name-based mesh<->bone matching for those parts. g.word()
        # already consumes exactly name_len bytes and stops at NUL, so the
        # cap never affected byte positioning — only the stored string.
        bone.name = g.word(name_len)
        bone.parent_id = w[2]
        g.b(1)
