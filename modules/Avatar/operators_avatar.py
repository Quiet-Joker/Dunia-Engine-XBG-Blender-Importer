...
from .binary_avatar import detect_endian_from_bytes, encode_chunk_magic, LE, BE
from ..Core.prefs import get_prefs
from ..Core.settings import XBGMatTemplateItem, _TEMPLATE_DESCRIPTIONS
from .import_xbg_avatar import XBGBlenderImporter
from .inject_xbg_avatar import XBGMeshInjector, calculate_required_scale
from .import_lks_avatar import parse_lks_file, create_lks_armature
from .import_mab_avatar import apply_multi_bone as _mab_apply, parse_sections as _mab_parse_sections
from . import export_materials_avatar as _export_materials
...
class XBG_OT_Import(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        # Import Behavior — moved here from the sidebar "Import Behavior"
        # panel (2026-06-30) so all import-time choices live in one place,
        # this file-select popup. These are scene-level settings
        # (xbg_debug_settings), read back in execute(); they persist across
        # imports, unlike the per-import operator props above.
        ds = context.scene.xbg_debug_settings
        box = layout.box()
        box.label(text="Import Behavior:", icon='TOOL_SETTINGS')
        box.prop(ds, "flip_normals",        text="Flip Normals on Import")
        box.prop(ds, "auto_smooth_normals", text="Auto-Smooth Normals")
        box.prop(ds, "separate_primitives", text="Separate Primitives (needed for inject)")
        box.prop(ds, "use_xml_assembly",    text="Use XML Assembly (weapons / vehicles)")

        box = layout.box()
        box.label(text="Texture Format:", icon='TEXTURE')
        box.prop(ds, "import_xbt_as_dds", text="Import XBT as DDS (not PNG)")
...
class XBG_OT_ClearSessionXBG(bpy.types.Operator):
    """Clear the pinned session XBG."""
    bl_idname = "xbg.clear_session_xbg"
...
class XBG_OT_AutoScaleBounds(bpy.types.Operator):
    ...
    bl_idname = "xbg.auto_scale_bounds"
...
class XBG_OT_InjectMesh(bpy.types.Operator):
    bl_idname = "xbg.inject_mesh"
...
class XBG_OT_ExpandBoundsForInject(bpy.types.Operator):
    ...
    bl_idname = "xbg.expand_bounds_for_inject"
...
class XBG_OT_SaveFormatBoundsSize(bpy.types.Operator):
    bl_idname = "xbg.save_format_bounds_size"
...
class XBG_OT_ImportLKSSkeleton(bpy.types.Operator):
    """Import a Ubisoft LKS binary skeleton file as a Blender armature."""
    bl_idname  = "xbg.import_lks_skeleton"
...
class XBG_OT_ImportMAB(bpy.types.Operator):
    """Import a .mab animation onto the selected armature."""
    bl_idname  = "xbg.import_mab_animation"
    ...
    def execute(self, ctx):
        ...
        try:
            ...
            msg = (f"MAB: {os.path.basename(self.filepath)}  "
                   f"{n_keyed}/{len(animated)} bones / "
                   f"{ctx.scene.frame_end} frames")

            # Scripted-scene files carry no character world-position data at
            # all (exhaustively confirmed - see agents.md "scripted-scene
            # character placement"). The closest the file format gets is its
            # own anchor system, so snap the armature there: without this the
            # character sits at Blender's origin while the scene's camera
            # (built separately via build_scene_objects) is positioned in
            # real level-world coordinates, hundreds of units away.
            from .mab_scene_avatar import find_scene_anchor_transform
            try:
                anchor = find_scene_anchor_transform(self.filepath,
                                                      arm.get('mab_anchor_override') or None)
            except Exception:
                anchor = None
            if anchor:
                loc, rot = anchor
                ...
                arm.location = loc
                arm.rotation_quaternion = rot
                msg += "  — snapped to scene anchor (set obj['mab_anchor_override'] to pick a different one)"
...
class XBG_OT_PreviewJiggle(bpy.types.Operator):
    ...
    def execute(self, ctx):
        ...
        if arm is None:
            ...
            return {'CANCELLED'}
        try:
            from .jiggle_avatar import load_jiggle_defs, bake_jiggle, PRESETS, _vec3
...
class XBG_OT_ImportLFA(bpy.types.Operator):
    """Import a .lfa facial pose library onto the selected armature."""
    bl_idname  = "xbg.import_lfa_poses"
    ...
    def execute(self, ctx):
        from .import_lfa_avatar import parse_lfa, apply_lfa_poses
        arm = ctx.active_object
        if arm is None or arm.type != 'ARMATURE':
...
class XBG_OT_ImportLFE(bpy.types.Operator):
    """Import a .lfe facial expression clip (needs the head's .lfa first)."""
    bl_idname  = "xbg.import_lfe_expression"
    ...
    def execute(self, ctx):
        from .import_lfa_avatar import parse_lfa, parse_lfe, apply_lfe_expression
        ds = ctx.scene.xbg_debug_settings
        arm = ctx.active_object
        if arm is None or arm.type != 'ARMATURE':
            arm = next((o for o in ctx.selected_objects
...
class XBG_OT_ScanSceneMAB(bpy.types.Operator):
    """Inspect a .mab's scripted-scene data without importing anything."""
    bl_idname  = "xbg.scan_scene_mab"
    ...
    def execute(self, ctx):
        from .mab_scene_avatar import scan_scene
        from .import_mab_avatar import (parse_sections as _ps, _stream_counts,
...
class XBG_OT_ImportSceneMAB(bpy.types.Operator):
    """Create the scene's anchors/cameras and event timeline markers."""
    bl_idname  = "xbg.import_scene_mab"
    bl_label   = "Import Scene Elements"
    bl_description = (
        "Build the scanned .mab's scene in Blender: an empty per anchor, a "
        "camera object per animated camera (with its full motion keyframed "
        "when the file has authored camera tracks - most clips only carry "
        "FOV cuts, in which case a static placeholder camera is created), "
        "and a timeline marker per timed event (sound/FX/dialog cues)"
    )
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, ctx):
        from .mab_scene_avatar import build_scene_objects
        ds = ctx.scene.xbg_debug_settings
        ...
        if not path or not os.path.isfile(path):
            ...
            return {'CANCELLED'}
        try:
            # MUST match the fps the character side resamples to
            # (apply_multi_bone / ds.mab_resample_fps), or the camera and
            # the character's body animation play out at different real
            # speeds even though both start at frame 1 — see
            # build_scene_objects' docstring. Covers the default/common
            # case (Smooth Resample ON, the default); if the user has
            # turned Smooth Resample OFF, apply_multi_bone keeps the
            # clip's native fps instead and this will still assume the
            # configured target — a known edge case, not handled here.
            r = build_scene_objects(ctx, path, fps=ds.mab_resample_fps)
...
class XBG_OT_NullSelectedVerts(bpy.types.Operator):
    """Move selected vertices to (0, 0, 0) so they export as zeroed HKX verts."""
    bl_idname  = "xbg.null_selected_verts"
