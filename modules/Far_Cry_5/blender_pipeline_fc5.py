...
`flip_face_winding`) are this folder's OWN copies (mesh_helpers_fc5) — no
...
from .binary_fc5 import LE
from ..Core.debug import VerboseLogger as vlog
from . import import_xbg_fc5 as import_xbg_fc3
from .mesh_helpers_fc5 import (apply_split_normals, store_tangent_attributes,
                               flip_face_winding)


# Version markers in the .xbg header (u32 at offset 4, after the 'HSEM' magic).
_VERSION_FC3 = 0x00030034   # Far Cry 3 (2012)
_VERSION_FC4 = 0x00060037
_VERSION_FC5 = 0x000D0047   # Far Cry 4 (2014)
...
def _load_fc3_or_fc4(ctx, fp, version, game_name, lod, separate, lhd):
    ...
    # own mesh_helpers_fc5 (module-level import above) — no avatar dependency.

    # Each skinned VB carries its OWN bone palette (vb['bone_palette']), resolved
    # by the parser from the SULC cluster data: si (per-vertex) indexes into that
    # VB's palette to get a global EDON bone index.  RE-validated geometrically
    # across FC5 head/newdawn/swat/body/top — see agents.md "FC5 skin palette".
    _has_palettes = bool(skinning and any(
        vb.get('bone_palette') for lod in lods for vb in lod['vbs']))
    vlog.log(f"SULC: {len(skinning.get('palettes', [])) if skinning else 0} "
             f"cluster palettes; per-VB assignment "
             f"{'active' if _has_palettes else 'unavailable'}")

    def _build_vert_pal(target):
        """Return list mapping global vert index → that vertex's bone palette.

        Prefers the per-vertex palette (`vb['vert_pal']`) so sections that share
        a VB but use different skin clusters (body vs arm-hair) each get their
        own palette; falls back to the VB-wide palette for older data.
        """
        result = []
        for vb in target['vbs']:
            vp = vb.get('vert_pal')
            if vp and len(vp) == len(vb['verts']):
                result.extend(vp)
            else:
                pal = vb.get('bone_palette')
                result.extend([pal] * len(vb['verts']))
        return result

    def _apply_vb_weights(obj, blender_vis, raw_verts, global_vis, vert_pal):
        ...
        global_vis   : global VB vert index for each entry (for palette lookup)
        vert_pal     : per-global-vert bone palette (list[int] or None)
        """
        if not bones:
            return 0
        assigned = 0
        ...
        for blender_vi, vert_d, gvi in zip(blender_vis, raw_verts, global_vis):
            sw = vert_d.get('sw')
            si = vert_d.get('si')
            # Some skinned meshes use up to 8 influences split across two sets
            # (BONE_WTS1 + BONE_WTS2); set1+set2 sum to 255.  Apply BOTH or the
            # second-set verts stay under-weighted and stretch.
            sw2 = vert_d.get('sw2')
            si2 = vert_d.get('si2')
            has1 = sw is not None and si is not None and sum(sw) != 0
            has2 = sw2 is not None and si2 is not None and sum(sw2) != 0
            if not has1 and not has2:
                continue
            palette = vert_pal[gvi] if gvi < len(vert_pal) else None
            if not palette:
                continue
            for cur_sw, cur_si in ((sw, si) if has1 else (None, None),
                                   (sw2, si2) if has2 else (None, None)):
                if cur_sw is None:
                    continue
                for k in range(4):
                    w_raw = cur_sw[k]
    ...
    for lod_idx in lods_to_import:
        ...
        # Build per-vert bone-palette map (used by both paths).
        vert_sulc = _build_vert_pal(target) if _has_palettes else []
        ...
        if separate:
            ...
            # FC5 ships authored R10G10B10A2 normals (and R10G10B10A2 tangents/
            # binormals) — apply them as custom split normals + round-trip attrs.
            # Must run AFTER flip_face_winding (its bmesh round-trip drops LOOP
            # custom normals; apply_split_normals also stores the POINT attr).
            if all_normals and len(all_normals) == len(positions):
                apply_split_normals(mesh, all_normals, False)
            if have_tangents or have_binormals:
                store_tangent_attributes(
                    mesh,
                    all_tangents if have_tangents else [],
                    all_binormals if have_binormals else [])
